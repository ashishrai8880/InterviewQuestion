...
// ======================================= 9. Minimise maximum distance between gas station ===============================
/*
...
stations[n-1]. Now, we add k more gas stations so that d, the maximum distance between adjacent gas stations, is minimized.
...
Output: 6.00 
Explanation: The largest gap is 14 (between 19 and 33). Adding 2 stations there splits it into approx 4.67. 
...
class Solution {
    ...
    minMaxDist(arr, k) {
        ...
        let n = arr.length ;
        let howManyPlaced = Array.from({length : n-1} , ()=>0);
        ...
        for(let j = 1 ; j<= k ; j++){
            ...
            for(let i=0 ; i<n-1 ; i++){
                ...
                const sectionLength = diff / (howManyPlaced[i]+1) ;
        ...
        for(let i =0 ; i<n-1 ; i++){
            const diff = arr[i+1] - arr[i];
            const sectionLength = (diff / (howManyPlaced[i]+1)).toFixed(2) ;
            ans = Math.max(ans , sectionLength ) ;
        }
        
        return ans == -Infinity ? 0.00 : ans  ;
